  ...
  class Base
    ...
    def import_cmd(file)
      if mysql?
        ...
        "#{pgpass} psql -c \"#{terminate_connection_sql};\" #{credentials} #{database}; #{pgpass} dropdb #{credentials} #{database}; #{pgpass} createdb #{credentials} #{database}; #{pgpass} psql #{credentials} #{database} < #{file}"
      end
    end
